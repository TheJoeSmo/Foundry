...
def meta_validator(meta_validator_function):
    return MetaValidator(meta_validator_function)


@fixture(scope="session")
def meta_validator_use_parent(meta_validator_function):
    return MetaValidator(meta_validator_function, use_parent=True)


@fixture(scope="session")
def meta_validator_not_use_parent(meta_validator_function):
    return MetaValidator(meta_validator_function, use_parent=False)


@fixture(scope="session")
def meta_validator_int(meta_validator_function):
    return MetaValidator(meta_validator_function, type_suggestion=int)


@fixture(scope="session")
def meta_validator_true(meta_validator_true_function):
    return MetaValidator(meta_validator_true_function)


@fixture(scope="session")
def meta_validate_false(meta_validator_false_function):
    return MetaValidator(meta_validator_false_function)


@fixture(scope="session")
def meta_validate_parent_test_use_parent(meta_validator_return_if_has_parent):
    return MetaValidator(meta_validator_return_if_has_parent, use_parent=True)


@fixture(scope="session")
def meta_validate_parent_test_not_use_parent(meta_validator_return_if_has_parent):
    return MetaValidator(meta_validator_return_if_has_parent, use_parent=False)


@fixture(scope="session")
def meta_validator_return_type_int(meta_validator_return_type_passed):
    return MetaValidator(meta_validator_return_type_passed, type_suggestion=int)


@fixture(scope="session")
def meta_validator_return_type_not_provided(meta_validator_return_type_passed):
    return MetaValidator(meta_validator_return_type_passed)
...
def test_type_handler_get_type_suggestion_from_class(meta_validator):
    assert TypeHandler({"test": meta_validator}, default_type_suggestion=int).get_type_suggestion("test") is int


def test_type_handler_get_type_suggestion_from_validator(meta_validator_int):
    assert TypeHandler({"test": meta_validator_int}, default_type_suggestion=str).get_type_suggestion("test") is int


def test_type_handler_get_type_suggestion_invalid():
    a = TypeHandler(default_type_suggestion=int)
    with raises(KeyError):
        a.get_type_suggestion("test")


def test_type_handler_has_type_meta_validator_function(meta_validator_function):
    assert TypeHandler({"test": meta_validator_function}).has_type("test")


def test_type_handler_has_type_meta_validator(meta_validator):
    assert TypeHandler({"test": meta_validator}).has_type("test")


def test_type_handler_has_type_not():
    assert not TypeHandler().has_type("test")


def test_type_handler_get_if_validator_uses_parent_use_parent(meta_validator_use_parent):
    assert TypeHandler({"test": meta_validator_use_parent}).get_if_validator_uses_parent("test")


def test_type_handler_get_if_validator_uses_parent_not_use_parent(meta_validator_not_use_parent):
    assert not TypeHandler({"test": meta_validator_not_use_parent}).get_if_validator_uses_parent("test")


def test_type_handler_get_if_validator_uses_parent_invalid():
    a = TypeHandler()
    with raises(KeyError):
        a.get_if_validator_uses_parent("test")


def test_type_handler_get_validator_meta_validator_function(meta_validator_function):
    assert TypeHandler({"test": meta_validator_function}).get_validator("test") is meta_validator_function


def test_type_handler_get_validator_meta_validator(meta_validator):
    assert TypeHandler({"test": meta_validator}).get_validator("test") is meta_validator.validator


def test_type_handler_get_validator_invalid():
    a = TypeHandler()
    with raises(KeyError):
        a.get_validator("test")


def test_type_handler_validate_by_type_simple(meta_validator_true):
    bool_handler = TypeHandler({"test": meta_validator_true}, default_type_suggestion=bool)
    assert TypeHandler.validate_by_type(
        "bool", {"type": "test"}, Namespace(validators=_TypeHandlerManager({"bool": bool_handler}))
    )


def test_type_handler_validate_by_type_use_parent(
    ...
    assert TypeHandler.validate_by_type(
        "bool", {"type": "use parent"}, Namespace(validators=_TypeHandlerManager({"bool": bool_handler}))
    )
    assert not TypeHandler.validate_by_type(
        "bool", {"type": "no parent"}, Namespace(validators=_TypeHandlerManager({"bool": bool_handler}))
    )


def test_type_handler_validate_by_type_passed(meta_validator_return_type_int, meta_validator_return_type_not_provided):
    handler = TypeHandler(
        {"int": meta_validator_return_type_int, "default": meta_validator_return_type_not_provided},
        default_type_suggestion=str,
    )
    assert (
        TypeHandler.validate_by_type(
            "types", {"type": "int"}, Namespace(validators=_TypeHandlerManager({"types": handler}))
        )
        is int
    )
    assert (
        TypeHandler.validate_by_type(
            "types", {"type": "default"}, Namespace(validators=_TypeHandlerManager({"types": handler}))
        )
        is str
    )


def test_type_handler_invalid_no_type_passed():
    with raises(ValueError):
        TypeHandler.validate_by_type("types", {}, Namespace(validators=_TypeHandlerManager({"types": TypeHandler()})))


def test_type_handler_invalid_type_passed():
    with raises(KeyError):
        TypeHandler.validate_by_type(
            "types", {"type": "test"}, Namespace(validators=_TypeHandlerManager({"types": TypeHandler()}))
        )


def test_type_handler_invalid_manager_passed():
    with raises(KeyError):
        TypeHandler.validate_by_type("types", {"type": "test"}, Namespace())


def test_type_handler_invalid_no_type_suggestion(meta_validator_return_type_not_provided):
    handler = TypeHandler({"test": meta_validator_return_type_not_provided})
    with raises(ValueError):
        TypeHandler.validate_by_type(
            "types", {"type": "test"}, Namespace(validators=_TypeHandlerManager({"types": handler}))
        )
