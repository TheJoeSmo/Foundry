...
class TestUndoRedoActor:
    __test_class__: type[UndoRedoActor] = UndoRedoActor
...
class TestUndoRedoForwarder(TestUndoRedoActor):
    __test_class__: type[UndoRedoForwarder] = UndoRedoForwarder
    ...
    def test_link_child(self):
        ...
        with SignalTester(obj.action_performed) as tester:
            obj2.action_performed.emit(Action(obj2, lambda: 1, lambda: 0, "set obj2 to 1"))
...
class TestUndoRedoRoot(TestUndoRedoActor, TestUndoRedo):
    ...
    def test_undo(self):
        undo_redo: SimpleUndoRedoRoot = self.__test_class__(0)
        undo_redo.do(Action(undo_redo, lambda: 1, lambda: 0, "1"))
        assert undo_redo.can_undo
        undo_redo.undo()
        assert 0 == undo_redo.model
    ...
    def test_redo(self):
        ...
        assert undo_redo.can_redo
        undo_redo.redo()
        assert 1 == undo_redo.model

    def test_inequality_with_undo(self):
        undo_redo: SimpleUndoRedoRoot = self.__test_class__(0)
        undo_redo2: SimpleUndoRedoRoot = self.__test_class__(0)
        undo_redo.do(Action(None, lambda: 1, lambda: 0, "1"), apply_action=True)
        undo_redo2.do(Action(None, lambda: 2, lambda: 0, "2"), apply_action=True)
        undo_redo.undo()
        undo_redo2.undo()
    ...
    def test_equality_with_redo_and_normal(self):
        ...
        undo_redo2: SimpleUndoRedoRoot = self.__test_class__(0)
        undo_redo.do(Action(None, lambda: 1, lambda: 0, "1"), apply_action=True)
        undo_redo2.do(Action(None, lambda: 1, lambda: 0, "1"), apply_action=True)
        undo_redo.undo()
        undo_redo.redo()
        assert undo_redo == undo_redo2
