...
@mark.parametrize(
    ...
    [
        ...
        (
            (slice(0x2010, 0x2020), False),
            bytearray([0xA0, 0xD3, 0x7B, 0xA2, 0x21, 0xA3, 0xA0, 0xD3, 0xA6, 0xA3, 0xDE, 0xA3, 0xAE, 0xA4, 0x2D, 0xAD]),
        ),
        (
            (slice(0x3C010, 0x3C020), False),
            bytearray([0x0, 0x60, 0xB0, 0x61, 0x60, 0x63, 0x10, 0x65, 0xC0, 0x66, 0x70, 0x68, 0x20, 0x6A, 0xD0, 0x6B]),
        ),
        ((slice(0x42010, 0x42011), True), bytearray([0x7F])),
        ((slice(0x4C010, 0x4C011), True), bytearray([0x00])),
        ((slice(0x6000F, 0x60010), True), bytearray([0xFE])),
    ],
)
def test_getitem(
