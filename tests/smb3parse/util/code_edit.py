...
def test_isValidNoGuards():
    test_area = CodeEdit(getTestRom(), 0x100, 1, empty_affix, empty_affix)
    assert True == test_area.isValid()

def test_isValid1BytePrefixInvalid():
    test_area = CodeEdit(getTestRom(), 0x100, 1, bytearray([0x11]), empty_affix)
    assert False == test_area.isValid()

def test_isValid1BytePostfixInvalid():
    test_area = CodeEdit(getTestRom(), 0x100, 1, empty_affix, bytearray([0x11]))
    assert False == test_area.isValid()

def test_longPrefix():
    test_area = CodeEdit(getTestRom(), 0x100, 1, long_prefix, empty_affix)
    assert True == test_area.isValid()

def test_longPostfix():
    test_area = CodeEdit(getTestRom(), 0x100, 1, empty_affix, long_postfix)
    assert True == test_area.isValid()

def test_fullAffix():
    test_area = CodeEdit(getTestRom(), 0x100, 1, long_prefix, long_postfix)
    assert True == test_area.isValid()
